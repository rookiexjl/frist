...
def URL(ip):
    uip=urllib.urlopen('http://wap.ip138.com/ip_search138.asp?ip=%s' % ip)
    fip=uip.read()
    rip=re.compile(r"<br/><b>(.*)</b><br/>")
    result=rip.findall(fip)
    print "%s\t %s" %(ip,result[0])


def DO(domain):
    url=urllib.urlopen('http://wap.ip138.com/ip_search138.asp?ip=%s' % domain)
    f=url.read()
    r=re.compile(r'&gt; (.*)<br/><b>(.*)</b><br/>')
    result=r.findall(f)
    # print type(result)
    for i in result:

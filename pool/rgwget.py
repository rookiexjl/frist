...
class RGWAdmin:
    ...
    def _load_request(self, r):
        ...
        if r.status_code == 200:
            ...
            for e in [exceptions.AccessDenied, exceptions.UserExists, exceptions.InvalidAccessKey,
                      exceptions.InvalidKeyType, exceptions.InvalidSecretKey, exceptions.KeyExists, 
                      exceptions.EmailExists, exceptions.SubuserExists, exceptions.InvalidAccess, 
                      exceptions.InvalidArgument,exceptions.IndexRepairFailed, exceptions.BucketNotEmpty,
                      exceptions.ObjectRemovalFailed, exceptions.BucketUnlinkFailed, exceptions.BucketLinkFailed,
                      exceptions.NoSuchObject, exceptions.InvalidCap, exceptions.NoSuchCap, 
                      exceptions.NoSuchUser, exceptions.NoSuchBucket, exceptions.NoSuchKey, 
                      exceptions.IncompleteBody, exceptions.BucketAlreadyExists, exceptions.InternalError]:
                if code == e.__name__:
                    raise e
            raise exceptions.RGWAdminException(code)
    ...
    def get_quota(self, uid, quota_type):
        if quota_type not in ['user', 'bucket']:
            raise exceptions.InvalidQuotaType
